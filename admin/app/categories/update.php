...
if (isset($_POST['name']) &&  isset($_POST['id'])) {
  ...
  //   $photo = uploadImage($conn,"photo","categories");
  // }else{
  //   $photo = $updated_file;
  // }

  $check = $conn->query("SELECT ID FROM categories WHERE (Name like '$name') AND ID <> $id");
  if ($check->num_rows > 0) {
    echo json_encode(['status' => 400, 'message' => $name . ' already exists!']);
    exit();
  }
   $add = $conn->query("UPDATE `categories` SET `Name` = '$name',`Slug` = '$slug' ,`Type` = '$type', `Position`='$position' WHERE ID = $id");
  if ($add) {
